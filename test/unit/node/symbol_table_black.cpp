 ...
 * ****************************************************************************
 *
 * Black box testing of cvc5pp::SymbolTable.
 */
...
{
  SymbolTable symtab;
  cvc5pp::Sort booleanType = d_tm.getBooleanSort();
  cvc5pp::Term x = d_tm.mkConst(booleanType);
  symtab.bind("x", x);
...
TEST_F(TestNodeBlackSymbolTable, bind2)
{
  ...
  cvc5pp::Term y = d_tm.mkConst(booleanType, "y");
  symtab.bind("x", y);
  ASSERT_TRUE(symtab.isBound("x"));
...
TEST_F(TestNodeBlackSymbolTable, bind3)
{
  ...
  cvc5pp::Term x = d_tm.mkConst(booleanType);
  symtab.bind("x", x);
  cvc5pp::Term y = d_tm.mkConst(booleanType);
...
TEST_F(TestNodeBlackSymbolTable, bind4)
{
  ...
  cvc5pp::Sort t = d_tm.mkUninterpretedSort("T");
...
{
  SymbolTable symtab;
  cvc5pp::Sort s = d_tm.mkUninterpretedSort("S");
  symtab.bindType("S", s);

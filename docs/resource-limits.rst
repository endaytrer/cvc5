...
The limits configured using :ref:`tlimit <lbl-option-tlimit>` and :ref:`rlimit <lbl-option-rlimit>` restrict time and resource usage over the whole lifetime of the :cpp:class:`solver <cvc5pp::Solver>` object, respectively.
In contrast to that, :ref:`tlimit-per <lbl-option-tlimit-per>` and :ref:`rlimit-per <lbl-option-rlimit-per>` apply to every check individually (:cpp:func:`checkSat <cvc5pp::Solver::checkSat>`, :cpp:func:`checkSatAssuming <cvc5pp::Solver::checkSatAssuming>`, etc).
...
Then, it returns `unknown` (with an :cpp:enum:`explanation <cvc5pp::UnknownExplanation>`).

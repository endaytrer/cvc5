...
class BagReduction
{
 public:
   ...
   * - (forall ((i Int))
   *    (let ((iMinusOne (- i 1)))
   *      (let ((elements_i (elements i)))
   *        (=>
   *          (and (>= i 1) (<= i n))
   *          (and
   *            (= (combine i) (f elements_i (combine iMinusOne)))
   *            (=
   *              (unionDisjoint i)
   *              (bag.union_disjoint
   *                (bag elements_i 1)
   ...
   * elements: Int -> T1 is an uninterpreted function that represents elements
   *           of A
   ...
   * - (forall ((i Int))
   *    (let ((elements_i (elements i)))
   *     (let ((count_elements_i (bag.count elements_i A)))
   *      (=>
   *       (and (>= i 1) (<= i n))
   *       (and
   *        (= (combine i) (+ count_elements_i (combine (- i 1))))
   *        (=
   *         (unionDisjoint i)
   *         (bag.union_disjoint
   *           (bag elements_i count_elements_i)
   *           (unionDisjoint (- i 1))))
   *        (forall ((j Int))
   *           (or
   *             (not (and (< i j) (<= j n)))
   *             (not (= (elements i) (elements j))))))))))
   * - (= (combine 0) 0)
   ...
   *   elements:Int -> T is an uninterpreted function that represents
   *            distinct elements of A
   *   combine: Int -> Int is an uninterpreted function
   *   unionDisjoint: Int -> (Bag T1) is an uninterpreted function
   */
